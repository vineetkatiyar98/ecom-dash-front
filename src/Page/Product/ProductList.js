...
const ProductList = () => {
  ...
  const getProducts = async () => {
    let result = await fetch("https://mern-dashboard-backend.cyclic.cloud/products", {
      headers: {
  ...
  const deleteProduct = async (id) => {
    console.warn(id);
    let result = await fetch(`https://mern-dashboard-backend.cyclic.cloud/product/${id}`, {
      method: "Delete",
    });
    result = await result.json();
    if (result) {
  ...
  const searchHandle = async (event) => {
    ...
    if (key) {
      let result = await fetch(`https://mern-dashboard-backend.cyclic.cloud/search/${key}`);
      result = await result.json();
      if (result) {
        setProducts(result);
      }
